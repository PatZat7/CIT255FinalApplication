...
namespace DeveloperDashboard
{
        ...
        public static decimal ValidateDecimalResponse(string promptMessage, string userResponse)
        {
            decimal userResponseInteger = 0;

            while (!(decimal.TryParse(userResponse, out userResponseInteger)))
            {
                ConsoleView.DisplayReset();
